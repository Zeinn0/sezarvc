...
from .vcp_helper import sezarvc
...
async def vc_reply(event, text, file=False, firstmsg=False, dlt=False, **kwargs):
    ...
    if vc_player.BOTMODE:
        try:
            if file:
                zedevent = await zedub.tgbot.send_file(
        ...
        try:
            ...
            zedevent = await results[0].click(event.chat_id, hide_via=True)
        except Exception:
            zedevent = await zedub.send_file(
                event.chat_id, file=file, caption=text, **kwargs
            )
